...
#pragma config(Servo,  srvo_S1_C2_1,    gyroServo,               tServoStandard)
...
void find_ir_basket(int motorspeed, int rotatemotorspeed, int angle)
{
	...
	motor[MotorL] = 0;
	motor[MotorR] = 0;
	basketDistance = find_basket_distance - 80;
}
...
{
	moveCm(backward, distance, motorpower);
	while (travelDistance != basketDistance)
	{
		move(forward, motorpower, 5);
		travelDistance ++;
		wait10Msec(1);
	}
}

void positionForRamp()
{
	moveCm(forward, 10, 50);
	rotate(right, 60, 135);
	moveCm(forward, 60, 50);
	rotate(left, 60, 90);
	moveCm(forward, 30, 90);
	rotate(left, 20, 0);
}

void simpleArc(int rightSpeed, int leftSpeed, int arcDistance)
{
	while(happy_angle < arcDistance)
	{
		motor[MotorR] = rightSpeed;
		motor[MotorL] = leftSpeed;
	}
}
